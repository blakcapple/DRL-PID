...
class EnvFeedback:
    def __init__(self):
        ...
        self.active = False
        self.image_sub = rospy.Subscriber("iRobot/camera/image_raw", Image, self.camera_callback)
    ...
    def camera_callback(self, data):
        ...
        cv_image = cv2.resize(cv_image, (img_width, img_height), interpolation=cv2.INTER_CUBIC)

        gray = cv2.cvtColor(cv_image, cv2.COLOR_BGR2GRAY)
        threshold = self.ostu(gray)
        threshold = min(180, threshold)
        ...
        if not self.detect_msg.failed_flag:
            ...
            for i in range(5):
                self.center_x[i] = (right_list[int(img_len * i / 4)] + left_list[int(img_len * i / 4)]) / 2
                self.center_y[i] = img_len / 4.0 * i

            for i in range(5):
                cv2.circle(mask, (int(self.center_x[i]), img_height - int(self.center_y[i])), 2, (0, 0, 0), -1)
