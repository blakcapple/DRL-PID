...
class LineFollower(object):
    ...
    def move_callback(self,data):
        ...
        # abs_e=0
        # for i in range(3):
        #     abs_e += abs(self.error[i])
        self.errorx_list.append(self.error[0])

    def _reward_calculate(self):

        error = 0.7*abs(self.error[0])+0.2*abs(self.error[1]) + 0.1*abs(self.error[2])
        ...
        if self.done :
            ...
            #      reward +=self.distance/(time.time()-self.time)
            # + 10*self.distance/(time.time()-self.time)
            print(reward)
